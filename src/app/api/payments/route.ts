...
export async function GET(request: Request) {
  ...
  const [rulesRes, occRes, txRes, categoriesRes, accountsRes] = await Promise.all([
    ...
    ctx.supabase
      .from("transactions")
      .select("id, scheduled_id, date, amount, payee")
  ...
  if (rulesRes.error) {
    if (isMissingRelationError(rulesRes.error.message)) {
      warning = "Brak tabeli scheduled_transactions — uruchom migracje na bazie PostgREST.";
    } else {
  ...
  const transactions = (
    txRes.error
      ? []
      : (txRes.data ?? []).filter(
          (row): row is { id: string; scheduled_id: string; date: string; amount: number; payee: string } =>
            Boolean(row.scheduled_id && row.date)
        )
  );
  if (txRes.error && !isMissingRelationError(txRes.error.message)) {
    warning = warning ? `${warning} ${txRes.error.message}` : txRes.error.message;
  }
