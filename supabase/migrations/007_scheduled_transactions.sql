-- Repair: live Coolify after 5944f87 returned
-- GET /api/cashflow?days=60&bucket=week → 500
-- (schema cache / relation "public.scheduled_transactions" does not exist).
-- 002_ynab_model.sql already declares this table, but boot can skip 002
-- (marked applied after a mid-file abort, or DATABASE_URL pointing at a
-- different database than PostgREST). Additive and safe to re-run.
