...
**Potwierdzony incydent (2026-09-09, deploy 5944f87):** `/budget` wrócił do 200, ale
...
- **`/payments` `column transactions.scheduled_id does not exist`:** ta sama własność tabeli — app `postgres` nie zrobi `ALTER`. Boot zawsze odpala `scripts/ensure-scheduled-id.sql` przez `DATABASE_OWNER_URL` / `SUPABASE_DB_URL` + `SET ROLE supabase_admin` + `NOTIFY pgrst`. UI nie pokazuje surowego angielskiego PostgREST — polski komunikat i `POST /api/setup/repair-scheduled`. Ręcznie: `psql -U supabase_admin -d postgres -f scripts/owner-add-scheduled-id.sql`.
