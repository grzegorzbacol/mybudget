...
- **Edytuj → Zapisz transfer 500 `transfer_account_id` … schema cache:** `010` po pierwszym deployu jest pomijane. Boot zawsze odpala `scripts/ensure-transfer-columns.sql` + `NOTIFY pgrst` na tym samym `DATABASE_URL` co PostgREST. PATCH/konwersja retry + zapis SQL gdy cache dalej stary. Ręcznie: `psql "$DATABASE_URL" -f supabase/migrations/011_transfer_account_id_schema_cache.sql`
